...
(function () {
    ...
    function file2string(file) {
        return `File(name=${file.name}, size=${formatFileSize(
            file.size
        )}, type=${file.type})`;
    }
    ...
    const plainFileInputLabel = document.getElementById(
        "plain-file-input-label"
    );
    ...
    function onNewEncryptedFile(file) {
        ...
        file.arrayBuffer()
            ...
            .catch((e) => {
                ...
                status.innerText =
                    "File decryption failed (incorrect password?)";
            });
    }
